...
/**
 * @swagger
 * /admin/dse/content/{id}/feature:
 *   post:
 *     summary: Feature DSE content
 *     description: Sets is_featured true (any status — visibility still gated on PUBLISHED).
 *     tags: [Admin]
...
export const POST = withErrorHandler(async (req: NextRequest, props: IdParams) => {
  ...
  return ok(await setAdminDseFeatured(id, adminId, true));
});
